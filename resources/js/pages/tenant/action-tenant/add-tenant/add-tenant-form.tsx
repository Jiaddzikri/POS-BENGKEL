...
export default function AddTenantForm({ tenant, status }: AddTenantFormProps) {
  const { data, setData, post, reset, errors, clearErrors } = useForm<FormTenant>({
    name: tenant?.name || '',
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div className="space-y-6 lg:col-span-2">
              <TenantBasicInformation action="Add" errors={errors} formData={data} status={status} handleInputChange={handleInputChange} />
            </div>
          </div>
